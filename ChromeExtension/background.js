...
TTSAudio.addEventListener('canplaythrough', function(event) { TTSAudio.play(); });

function requestSynthesize(textToSynthesize, voice)
{
   fetch('http://speech.ssn.edu.in/synthesise.php',
   {
      method: 'post',
      headers:
      {
   ...
   }).then(function(resTxt)
   {
      ...
      TTSAudio.src="http://speech.ssn.edu.in/wav/" + wavFileName;
   });
}

function synthesizeSelectedMale(info, tab) { requestSynthesize(info.selectionText, "hts_tamil_male"); }
function synthesizeSelectedFemale(info, tab) { requestSynthesize(info.selectionText, "hts_tamil_female"); }
